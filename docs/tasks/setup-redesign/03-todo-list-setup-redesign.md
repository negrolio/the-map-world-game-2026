...
**Estado:** **pendiente** — Fases 1–7 sin iniciar.
...
- [ ] En [`src/services/product-rules.ts`](../../../src/services/product-rules.ts), agregar:
  ```ts
  ai: { maxPlayers: 2, fixedQuestionCount: 5 }
  ```
- [ ] Helper opcional `getMaxPlayersForMode(questionMode: QuestionMode): number` (export desde `product-rules.ts` o `services/index.ts`).
- [ ] Actualizar [`src/services/product-rules.test.ts`](../../../src/services/product-rules.test.ts): asserts de `PRODUCT_RULES.ai` y helper si existe.
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — `npm run test -- src/services/product-rules.test.ts`

### Tarea 1.2 — `validateConfig` con reglas AI

- [ ] En [`src/services/validate-config.ts`](../../../src/services/validate-config.ts), cuando `questionMode === 'ai'`:
...
- [ ] Mantener reglas país/capital sin regresión (max 6 global).
- [ ] Ampliar [`src/services/validate-config.test.ts`](../../../src/services/validate-config.test.ts):
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — `npm run test -- src/services/validate-config.test.ts`

### Tarea 1.3 — Schema Zod alineado

- [ ] En [`src/features/setup/setup-config-schema.ts`](../../../src/features/setup/setup-config-schema.ts), agregar `.superRefine` para modo AI:
  - jugadores ≤ `PRODUCT_RULES.ai.maxPlayers`
  - `questionCount === PRODUCT_RULES.ai.fixedQuestionCount`
- [ ] Mensajes: `schema.aiPlayersMax`, `schema.aiFixedQuestionCount` (o convención alineada al repo).
- [ ] Ampliar [`src/features/setup/setup-config-schema.test.ts`](../../../src/features/setup/setup-config-schema.test.ts) con fixtures AI válidos/inválidos.
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — `npm run test -- src/features/setup/setup-config-schema.test.ts`

### Cierre de Fase 1

- [ ] **Review** — diff completo de `product-rules`, `validate-config`, `setup-config-schema` y tests; sin archivos fuera de alcance.
- [ ] **Buscar errores** — `npx tsc --noEmit` + `npm run lint`.
- [ ] **Testear** — `npm run test -- src/services/product-rules.test.ts src/services/validate-config.test.ts src/features/setup/setup-config-schema.test.ts`
- [ ] **Corregir** — cualquier fallo antes de Fase 2.
...
- [ ] [`src/i18n/resources/es.ts`](../../../src/i18n/resources/es.ts):
...
- [ ] Replicar en [`src/i18n/resources/en.ts`](../../../src/i18n/resources/en.ts) con copy del PRD §2 / decisión D3.
...
- [ ] Borrar de ES + EN: `setup.validConfig`, `setup.configPreviewHeading`, `setup.aiStrictRequired`, `setup.questionModeLegend`.
- [ ] Grep en repo: confirmar cero referencias a claves eliminadas (`rg 'validConfig|configPreviewHeading|aiStrictRequired|questionModeLegend'`).
- [ ] Actualizar consumidores que aún referencien claves viejas (p. ej. [`SetupView.tsx`](../../../src/features/setup/SetupView.tsx) si compila antes de Fase 4 — puede requerir stub temporal o hacer en la misma tarea).
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — `npx tsc --noEmit` + grep sin referencias rotas

### Cierre de Fase 2

- [ ] **Review** — solo `es.ts`, `en.ts` y fixes de referencias; copy ES/EN pareado.
- [ ] **Buscar errores** — `npx tsc --noEmit` + `npm run lint`.
- [ ] **Testear** — `npm run test` (suite completa por si App/Setup aún referencian claves); corregir roturas de compilación.
- [ ] **Corregir** — antes de Fase 3.
...
- [ ] Crear [`src/features/setup/SetupModeCard.tsx`](../../../src/features/setup/SetupModeCard.tsx) basado en [`HomeModeCard.tsx`](../../../src/features/home/HomeModeCard.tsx):
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — tests de Tarea 3.3 si ya existen; si no, `tsc` + render manual

### Tarea 3.2 — `SetupModeCardGroup`

- [ ] Crear grupo (mismo archivo o [`SetupModeCardGroup.tsx`](../../../src/features/setup/SetupModeCardGroup.tsx)):
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — tests Tarea 3.3

### Tarea 3.3 — Export y tests unitarios

- [ ] Export en [`src/features/setup/index.ts`](../../../src/features/setup/index.ts).
- [ ] Crear [`src/features/setup/SetupModeCard.test.tsx`](../../../src/features/setup/SetupModeCard.test.tsx):
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — `npm run test -- src/features/setup/SetupModeCard.test.tsx`

### Cierre de Fase 3

- [ ] **Review** — componentes aislados; sin wiring en SetupView aún (o mínimo).
- [ ] **Buscar errores** — `npx tsc --noEmit` + `npm run lint`.
- [ ] **Testear** — `npm run test -- src/features/setup/SetupModeCard.test.tsx`
- [ ] **Corregir** — antes de Fase 4.
...
- [ ] Refactor [`src/features/setup/SetupView.tsx`](../../../src/features/setup/SetupView.tsx):
...
- [ ] Eliminar preview JSON y rama `validConfig`.
- [ ] Quitar prop `setupDraft` de `SetupViewProps` y call site en [`App.tsx`](../../../src/App.tsx) si ya no se usa.
- [ ] Quitar `FieldRadioGroup` de modo (reemplazado por cards en lobby).
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — `tsc` + tests SetupView Tarea 4.3

### Tarea 4.2 — Toast de recorte AI (UI)

- [ ] Prop `setupNotice: string | null` en `SetupViewProps`.
- [ ] Render `Alert tone="info"` con `aria-live="polite"` cuando hay notice.
- [ ] Auto-ocultar ~4–5 s (`useEffect` + timeout en SetupView o coordinado con App en Fase 5).
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — test SetupView: notice visible y desaparece (fake timers)

### Tarea 4.3 — Responsividad y props dinámicas

- [ ] `grid grid-cols-3 gap-2 md:gap-4` en cards.
- [ ] Prop `playerMax` para `max` del input de jugadores (wired en Fase 5; stub `6` hasta entonces si hace falta).
- [ ] Verificar layout en viewport 390px (test Vitest con `innerWidth` opcional).
...
- [ ] Crear [`src/features/setup/SetupView.test.tsx`](../../../src/features/setup/SetupView.test.tsx):
...
- [ ] Buscar errores
- [ ] Corregir
- [ ] Testear — `npm run test -- src/features/setup/SetupView.test.tsx`

### Cierre de Fase 4

- [ ] **Review** — SetupView + App (props); lobby fuera del pergamino; sin JSON/éxito.
- [ ] **Buscar errores** — `npx tsc --noEmit` + `npm run lint`.
- [ ] **Testear** — `npm run test -- src/features/setup/`
- [ ] **Corregir** — actualizar [`App.test.tsx`](../../../src/App.test.tsx) si ya falla por copy/layout (mínimo para compilar).
...
- [ ] En [`src/App.tsx`](../../../src/App.tsx), reemplazar `onQuestionModeChange={setQuestionMode}` por handler:
...
- [ ] `effectiveMax = getMaxPlayersForMode(questionMode)` (o ternario con `PRODUCT_RULES.ai.maxPlayers`).
- [ ] Aplicar bound en lugar de `PRODUCT_RULES.players.max` fijo.
...
- [ ] `setupNotice` state en App; pasar a `SetupView`.
- [ ] Pasar `playerMax` a SetupView.
- [ ] Limpiar notice al cambiar modo (opcional: al iniciar partida).
- [ ] Confirmar `effectiveAntiCheatMode` sin cambios.
...
- [ ] Setup: ya no "Modo de preguntas" / "Cobertura geográfica"; sí "Elige un continente".
- [ ] Dos botones "Jugar ahora".
- [ ] AI: 4 jugadores en país → card AI → 2 jugadores + notice.
- [ ] AI: sin input "Número de preguntas".
...
- [ ] **Review** — App handlers + props SetupView; lógica AI acotada.
- [ ] **Buscar errores** — `npx tsc --noEmit` + `npm run lint`.
- [ ] **Testear** — `npm run test -- src/App.test.tsx src/features/setup/`
- [ ] **Corregir** — antes de Fase 6.
...
- [ ] [`e2e/smoke.spec.ts`](../../../e2e/smoke.spec.ts) — CTA "Jugar ahora".
- [ ] [`e2e/game-flow.spec.ts`](../../../e2e/game-flow.spec.ts) — CTA + sin radios de modo / textos eliminados.
- [ ] [`e2e/ai-trivia-flow.spec.ts`](../../../e2e/ai-trivia-flow.spec.ts) — idem.
- [ ] [`e2e/learn-flow.spec.ts`](../../../e2e/learn-flow.spec.ts) — si referencia setup copy.
...
- [ ] Crear [`e2e/setup-redesign.spec.ts`](../../../e2e/setup-redesign.spec.ts) (o ampliar `game-flow`):
...
- [ ] Callout superseding al inicio de [`docs/requirements/01-prd-mvp-producto-y-requerimientos.mdc`](../../requirements/01-prd-mvp-producto-y-requerimientos.mdc) §RF-01.
- [ ] Actualizar [`docs/tasks/setup-redesign/README.md`](./README.md): estado cerrado, entregables, verificación.
- [ ] Mover entrada en [`docs/tasks/ideas-features-backlog.md`](../ideas-features-backlog.md) → **Cerradas**.
- [ ] Actualizar [`docs/requirements/04-current-state-post-mvp.mdc`](../../requirements/04-current-state-post-mvp.mdc) §1.
- [ ] Marcar [`01-prd-setup-redesign.md`](./01-prd-setup-redesign.md) como **cerrado** (fecha merge).
